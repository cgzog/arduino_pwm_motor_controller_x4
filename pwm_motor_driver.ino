// pwm motor driver
//
// this is intended to drive a Rev Robotics SparkMAX (or equiv) motor controller using the PWM inputs
//
...
// sets speed - direction is controlled by a DPDT center-off switch.
// Each of the positions is connected to a digital input and run the motor in the desired direction
// at the speed proportional to the pot setting - when no speed is selected, the inputs are pulled high
// using on-board pullup in the micro controller and the motor is stopped via the appropriate PWM signal.
